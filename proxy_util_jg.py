...
def get_proxy_source_df():
    ...
    while go_ahead:
        ...
        if page_num > last_page:
            go_ahead = False
        time.sleep(0.89)
    df_proxy_source_jg = pd.DataFrame(source_area_list, columns=['province', 'province_code', 'city', 'city_code'])
    df_proxy_source_jg['province_code'] = df_proxy_source_jg['city_code'].apply(lambda x: re.sub(r'(?<=\d{2})(\d{4})','0000', x))
    return df_proxy_source_jg
...
class ProxyUtil_JG:
    ...
    def get_proxy_from_jg(self,num,province_code,city_code,level_code):
        if level_code == 1:
            api_jg = f'http://d.jghttp.alicloudecs.com/getip?num={num}&type=2&pro={province_code}&city={city_code}&yys=0&port=11&pack=35822&ts=1&ys=0&cs=1&lb=1&sb=0&pb=45&mr=1&regions='
        elif level_code == 2:
            api_jg = f'http://d.jghttp.alicloudecs.com/getip?num={num}&type=2&pro=0&city=0&yys=0&port=11&pack=35822&ts=1&ys=0&cs=1&lb=1&sb=0&pb=45&mr=1&regions={province_code}'
        else:
            api_jg = f'http://d.jghttp.alicloudecs.com/getip?num={num}&type=2&pro=0&city=0&yys=0&port=11&pack=35822&ts=1&ys=0&cs=1&lb=1&sb=0&pb=45&mr=1&regions={province_code}'
        resp_ip = requests.get(api_jg)
    ...
    def enrich_proxy_list(self,proxy_jg):
        # self.proxy_logger(f"keys in dynamic_proxy_table:{self.dynamic_proxy_table.keys()}")
        proxy_list_city = []
        proxy_list_province = []
        for proxy_data in proxy_jg:
            proxy_str = ':'.join([proxy_data['ip'],str(proxy_data['port'])])
            proxy_data['banned_times'] = 0
            proxy_city_jg = proxy_data['city']
            se_picked = self.df_proxy_source_from_jg[self.df_proxy_source_from_jg['city'].str.contains(rf'\S?{proxy_city_jg}\S?')].head(1)
            if se_picked.shape[0] == 1:
                city_code_jg = se_picked.iloc[0]['city_code']
                province_code_jg = se_picked.iloc[0]['province_code']
                if city_code_jg in self.dynamic_proxy_table.keys():
                    proxy_list_city = self.dynamic_proxy_table[city_code_jg]
                if province_code_jg in self.dynamic_proxy_table.keys():
                    proxy_list_province = self.dynamic_proxy_table[province_code_jg]
                self.proxy_logger(f'{proxy_str} join in,from city {proxy_city_jg}')
            else:
        ...
        self.dynamic_proxy_table[city_code_jg] = proxy_list_city
        self.dynamic_proxy_table[province_code_jg] = proxy_list_province
    ...
    def output_proxy(self,cur_adcode,cur_header,cur_url):
        ...
        if self.df_proxy_source_from_jg[self.df_proxy_source_from_jg['city_code'] == cur_city_code].shape[0] > 0:
            level_code = 1
            key_code = cur_city_code
        elif self.df_proxy_source_from_jg[self.df_proxy_source_from_jg['province_code'] == province_code].shape[0] > 0:
            level_code = 2
        ...
        if key_code in self.dynamic_proxy_table.keys():
            ...
            if len(proxy_list_tmp) < 3:
                try:
                    ...
                    enrich_res = self.get_proxy_from_jg(3-len(proxy_list_tmp), province_code, cur_city_code,level_code)
                    if enrich_res != '0':
                        if enrich_res == '121':
                            return enrich_res
                        else:
            ...
            if proxy_checked is None:
                ...
                if now > expire_time:
                    ...
                    try:
                        self.proxy_logger(f'enrich proxy_pool for {key_code} *****')
                        enrich_res = self.get_proxy_from_jg(1, province_code, cur_city_code,level_code)
                        if enrich_res != '0':
                            if enrich_res == '121':
                                return enrich_res
                            else:
            ...
            try:
                ...
                enrich_res = self.get_proxy_from_jg(3,province_code,cur_city_code,level_code)
                if enrich_res == 0:
                    self.output_proxy(cur_adcode,cur_header,cur_url)
